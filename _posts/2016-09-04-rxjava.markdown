...
# Rxjava 原理探究 #
关于 Rxjava 的文章网上有很多，这里只是个人的学习总结，阅读前需要对 Rxjava 有一定的了解，并不能作为学习资料，如果想要入门这里有更好的选择（虽然版本有点久，很多 API 都已过时，但原理差不多，不影响理解）：[给 Android 开发者的 RxJava 详解](https://gank.io/post/560e15be2dca930e00da1083)
...
```
简单解释下这段代码：观察者发出数据源，然后对数据源进行三次数据类型变换，之后指定被观察者和订阅者所在线程，最后对观察者进行订阅。
...
- `Comsumer`（消费者） 和 `Observer`（观察者） 及其子类统称为 **订阅者**（两者在 Rxjava 含义相似）；
- 由于 Rxjava 订阅的 API 在 `Observale` 中，给我们的感觉是 **被观察者** 订阅 **观察者**，因此为了叙述方便和避免含义混乱，**被观察者** 订阅 **观察者** 和 **观察者** 订阅 **观被察者** 是同样的意思；
- 本文基于 Rxjava 当前最新版本 2.0.7 。
...
public final class ObservableMap<T, U> extends AbstractObservableWithUpstream<T, U> {
    ...
    public void subscribeActual(Observer<? super U> t) {  // 5
        //新建一个 MapObserver 订阅者，用来订阅实际 Observable 对象 source
        source.subscribe(new MapObserver<T, U>(t, function)); // 6
    }
    //此类的构造方法传入的参数是变换器 mapper 和真实订阅者 actual
    static final class MapObserver<T, U> extends BasicFuseableObserver<T, U> {
        ...
        public void onNext(T t) { // 7
            ...
            U v;
            try {
                //通过转换器把从原 Observable 对象发出的数据转化成真实订阅者要求的数据
...
```
因为代码不多，我全部贴上来，便于大家理解。在上面的过程中，有这几类角色：1）实际被观察者；2）新创建的被观察者；3）实际订阅者；4）新创建的订阅者；5）转换器 mapper。它们的关系比较复杂，我们按照代码的执行顺序来将思路理一遍：

- `Observable` 通过 `just()` 这个静态工厂方法创建了一个 `Observable` 对象，此对象即“实际被观察者”；
- `map()` 方法接受一个转换器对象，在内部新建了一个被观察者后将其返回，也就是 “新创建的被观察者”；
- 用 map 变换后返回的那个 “新创建的被观察者” 订阅 “实际订阅者”，最终 “新创建的被观察者” 的 `subscribeActual()` 方法被调用，此方法触发的操作有：
 - 创建一个订阅者，也就是 “新创建的订阅者”，向 “实际的被观察者” 订阅；
 - 实际的被观察者发出数据，通过调用 “新创建的订阅者” 的 `onNext()` 方法向其传递数据；
 - 在新创建的被订阅者的内部先用 “转换器” 的 `apply()` 方法将数据类型进行转换，然后调用 “实际订阅者” 的 `onNext()` 方法将转换后的数据传给 “实际订阅者”
...
说明：
- A 表示实际被观察者，B 表示新创建的订阅者，C 表示新创建的被观察者，D 表示实际的订阅者，E 代表转换器；
- 虚线框是 map 变换导致的新创建的角色，它们没有直接接触并且未发生任何订阅关系；
- B 和 D 都是订阅者，它们进行了直接的接触：B 将信息转换后交给 D;
